...
public class JoMaloneInvEmail {
	...
	public void nexusChat(String orderID,String email) throws InterruptedException, AWTException {
		...
		//To verify the msg which ask to enter the email
				...
				String invMsg=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@id=\"lp_line_bubble_8\"]/div"))).getText();
				System.out.println(invMsg);
				String inv1="The email address entered isn't valid, please check it is the one used to place the order and that it is a valid format for example:";
				String inv2="It looks like the email address you entered isn't valid. Please double-check that it’s the one used to place the order and follows the correct format, like your.name@email.com.\r\n";
				if(invMsg.contains(inv1)||invMsg.contains(inv2))
				{
					System.out.println("Invalid msg Reply is Matched");
				}
				else {
					System.out.println("Invalid msg is Mismatched");
				}
